...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>KodingNow | Detail Work</title>
    ...
    <body x-data="{  }">
        ...
        <section id="work" class="py-20">
            <div class="container">
                <h2 class="font-semibold text-xl text-center md:text-3xl">{{ $work != null ? $work->title : 'no_title' }}</h2>
                <p class="text-center text-gray-700 mb-10">{{__("words.section_title_work")}}</p>

                @if ($work != null)
                <div class="w-full lg:w-1/2 mx-auto mb-10">
                    <div class="rounded-lg overflow-hidden">
                        <img class="object-cover" src="{{ $work->image_url }}" alt="Our Works Image">
                    </div>
                </div>
    
                <div class="flex flex-col gap-4">
                    <h5 class="text-xl font-semibold">{{__("words.description")}}:</h5>
                    <p class="text-gray-700">{{ $work->description }}</p>
                    <h5 class="mb-10">{{__("words.year")}}: {{ $work->year }}</h5>
                    <div>
                        <a href="{{ $work->result_link }}" target="_blank" class="btn-purple">{{__("words.view_result")}}</a>
                    </div>
                </div>
                @else
                    <div class="w-full mx-auto bg-red-200 rounded-full py-3 px-6 md:w-1/2 lg:w-1/3">
                        <p class="text-red-400">Work not found.</p>
                    </div>
                @endif
            </div>
        </section>
